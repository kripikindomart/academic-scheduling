...
class UpdateClassRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'program_study_id' => 'sometimes|required|exists:program_studies,id',
            'batch_year' => 'sometimes|required|integer|min:2000|max:' . (date('Y') + 10),
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Check if capacity reduction would affect current students
            $class = $this->route('class');
            if ($class && $this->capacity && $this->capacity < $class->current_students) {
                $validator->errors()->add('capacity', 'Kapasitas tidak dapat dikurangi karena ada ' . $class->current_students . ' mahasiswa yang terdaftar.');
            }
        });
    }
}