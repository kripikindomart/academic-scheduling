      ...
      >
        ...
        <div class="flex min-h-full items-center justify-center p-4">
          ...
          >
            <div
              v-if="show"
              class="relative mx-auto max-w-lg w-full transform rounded-2xl shadow-2xl transition-all"
              @click.stop
            >
              ...
              <div class="relative overflow-hidden rounded-2xl bg-white shadow-xl">
                ...
                <div class="px-6 py-6">
                  <div class="text-center">
                    <p class="text-gray-600">
                      Apakah Anda yakin ingin menghapus {{ itemCount }} kelas secara massal?
                    </p>

                    <!-- Warning Details -->
                    <div class="mt-4 rounded-lg bg-orange-50 p-4">
                      <div class="flex items-start space-x-3">
                        <svg class="h-5 w-5 text-orange-600 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
                        </svg>
                        <div class="text-left">
                          <p class="text-sm font-medium text-orange-800">
                            <strong>Perhatian:</strong> Data yang dihapus akan dipindahkan ke trash dan dapat dipulihkan kembali.
                          </p>
                          <p v-if="hasActiveStudents" class="text-sm text-orange-700 mt-1">
                            Kelas dengan mahasiswa aktif tidak dapat dihapus.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div v-if="itemList && itemList.length > 0" class="mt-4 max-h-32 overflow-y-auto">
                      <div class="text-xs text-gray-500 text-left mb-2">Item yang akan dihapus:</div>
                      <div class="bg-gray-50 rounded-lg p-2 text-left">
                        <div v-for="(item, index) in itemList.slice(0, 5)" :key="index" class="text-xs text-gray-600">
                          • {{ item.name || item.code || `Item ${index + 1}` }}
                        </div>
                        <div v-if="itemList.length > 5" class="text-xs text-gray-500 mt-1">
                          ... dan {{ itemList.length - 5 }} item lainnya
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div class="bg-gray-50 px-6 py-4">
                  <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 sm:space-x-reverse space-y-3 space-y-reverse sm:space-y-0">
                    ...
                    <button
                      type="button"
                      class="inline-flex w-full justify-center rounded-lg border border-transparent bg-gradient-to-r from-orange-600 to-red-600 px-4 py-3 text-base font-medium text-white shadow-lg hover:from-orange-700 hover:to-red-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-all duration-200 transform hover:scale-105"
                      @click="handleConfirm"
                      :disabled="loading"
                    >
                      <span class="flex items-center">
...
const props = defineProps({
  ...
  title: {
    type: String,
    default: 'Konfirmasi Hapus Massal'
  },
  message: {
    type: String,
    default: ''
