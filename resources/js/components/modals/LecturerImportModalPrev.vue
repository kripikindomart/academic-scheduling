                ...
                <div v-if="validationResults.all_data && validationResults.all_data.length > 0" class="space-y-4">
                  ...
                  <div class="bg-gray-100 px-4 py-2 border-b border-gray-200 flex items-center justify-between">
                    <div class="text-xs text-gray-600 font-mono">
                      <span class="text-blue-600">A1:{{ String.fromCharCode(65 + 24) }}{{ validationResults.all_data.length + 1 }}</span> •
                      <span class="font-medium">{{ validationResults.all_data.length }} rows</span> ×
                      <span class="font-medium">25 columns</span>
                    </div>
                    <div class="flex gap-2">
                      <button @click="exportToCSV" class="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200">
                        📊 Export
                      </button>
                      <button @click="selectAllData" class="text-xs px-2 py-1 bg-green-100 text-green-700 rounded hover:bg-green-200">
                        ⬚ Select All
                      </button>
                    </div>
                  </div>
                  ...
                  <div class="border border-gray-300 rounded-lg overflow-hidden" style="height: 500px;">
                    ...
                    <div class="sticky top-0 z-10 bg-gray-50 border-b border-gray-300">
                      <div class="flex overflow-hidden" style="width: 2500px;">
                        <!-- Row Number Column -->
                        <div class="flex-shrink-0 w-12 px-1 py-2 text-xs font-medium text-gray-700 text-center border-r border-gray-300 bg-gray-100">#</div>
                        <!-- Status Column -->
                        <div class="flex-shrink-0 w-20 px-1 py-2 text-xs font-medium text-gray-700 border-r border-gray-300">Status</div>
                        <!-- Other Columns -->
                        <div v-for="(header, index) in excelHeaders" :key="index" class="flex-shrink-0 px-2 py-2 text-xs font-medium text-gray-700 border-r border-gray-300" :style="getColumnWidth(index)">
                          {{ header }}<span v-if="requiredFields.includes(header)" class="text-red-500">*</span>
                        </div>
                      </div>
                    </div>

                    <!-- Scrollable Data Rows -->
                    <div class="overflow-auto" style="height: 450px; width: 100%;">
                      <div v-for="(item, rowIndex) in validationResults.all_data" :key="rowIndex"
                           class="flex border-b border-gray-200 hover:bg-blue-50 transition-colors"
                           style="width: 2500px;"
                           :class="getRowClass(item)">
                        <!-- Row Number -->
                        <div class="flex-shrink-0 w-12 px-1 py-1 text-xs text-gray-600 text-center border-r border-gray-300 bg-gray-50">
                          {{ item.row_number }}
                        </div>

                        <!-- Status -->
                        <div class="flex-shrink-0 w-20 px-1 py-1 border-r border-gray-300">
                        ...
                        <!-- Editable Data Cells -->
                        <div v-for="(header, colIndex) in excelHeaders" :key="colIndex"
                             class="flex-shrink-0 px-1 py-1 border-r border-gray-300"
                             :style="getColumnWidth(colIndex)">
                          <input
                            v-model="item.mapped_data[getFieldKey(header)]"
                            type="text"
                            class="w-full px-1 py-0.5 text-xs border-0 rounded focus:ring-1 focus:ring-blue-500 focus:bg-white"
                            :class="getCellClass(item, header)"
                  ...
                  <!-- Excel Footer Bar -->
                  <div class="bg-gray-100 px-4 py-2 border-t border-gray-200 flex items-center justify-between">
                    <div class="text-xs text-gray-600">
                      💡 <span class="font-medium">Tips:</span> Klik sel untuk edit, gunakan <kbd>Tab</kbd> untuk navigasi, <kbd>Ctrl+Z</kbd> untuk undo
                    </div>
                    <div class="text-xs text-gray-600">
                      Scroll ke kanan untuk lihat semua kolom →
                    </div>
                  </div>
                </div>
...
// Excel Configuration
const excelHeaders = [
  'Nama', 'Email', 'NIP/NIDN', 'No HP', 'No KTP', 'Jenis Kelamin',
  'Tempat Lahir', 'Tanggal Lahir', 'Alamat', 'Kota', 'Provinsi', 'Kode Pos',
  'Kebangsaan', 'Agama', 'Gol Darah', 'Status Pegawai', 'Jenis Pegawai',
  'Status Dosen', 'Tanggal Masuk', 'Jabatan', 'Gelar', 'Bidang Keahlian',
  'Departemen', 'Fakultas', 'Pendidikan Tertinggi'
]

const requiredFields = ['Nama', 'Email', 'NIP/NIDN', 'No HP', 'Status Pegawai', 'Jenis Pegawai', 'Status Dosen']
...
const getColumnWidth = (index) => {
  const widths = ['120px', '150px', '100px', '120px', '100px', '100px', '120px', '100px', '80px', '120px', '150px', '80px', '120px', '100px', '100px', '140px', '120px', '120px', '120px', '150px', '120px', '180px', '150px', '160px']
  return `width: ${widths[index] || '120px'}; min-width: ${widths[index] || '120px'};`
}
...
const getFieldKey = (header) => {
  const fieldMap = {
    ...
    'Status Pegawai': 'status_kepegawaian_wajib',
    'Jenis Pegawai': 'jenis_pegawai_wajib',
    'Status Dosen': 'status_dosen_wajib',
...
const getPlaceholder = (header) => {
  const placeholders = {
    ...
    'NIP/NIDN': '1234567890123456',
    'No HP': '081234567890',
    'Status Pegawai': 'PNS/PNS DPK/CPNS/Non-PNS',
    'Jenis Pegawai': 'Tetap/Kontrak/Paruh Waktu/Tamu',
    'Status Dosen': 'Aktif/Cuti/Tidak Aktif'
  }
  return placeholders[header] || ''
}

const getCellClass = (item, header) => {
  const fieldKey = getFieldKey(header)
  const hasError = item.errors_array?.some(error =>
    error.toLowerCase().includes(fieldKey) ||
  ...
  if (!item.mapped_data[fieldKey] && requiredFields.includes(header)) {
...
const selectAllData = () => {
  toastStore.info('Fitur select all akan segera hadir')
}
...
const exportToCSV = () => {
  ...
  const csv = [
    excelHeaders.join(','),
    ...validationResults.value.all_data.map(item =>
      excelHeaders.map(header =>
        `"${item.mapped_data[getFieldKey(header)] || ''}"`
      ).join(',')
    )
  ].join('\n')
...
// Quick Fix Functions (same as before)
const autoFillProgramStudy = () => {
  if (!validationResults.value.all_data) return

  const defaultProgramStudy = 'Teknik Informatika'
  let filledCount = 0
